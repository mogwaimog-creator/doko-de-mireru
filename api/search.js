...
if (provider === "unext") {
  ...
  return res
    ...
    .json({
      results:
        unextResults.results,

      page:
        unextResults.page,

      hasMore:
        unextResults.hasMore
    });
...
function removeDuplicatePersonWorks(
  works
) {
...
Object.keys(titleAliasMap)
  .forEach(function(key){

    if(
      aliasKey === normalizeTitle(key)
    ){
...
if (
  normalized === "かぜとともにさりぬ" ||
  normalized === "カゼトトモニサリヌ" ||
  normalized === "風と共に去りぬ"
) {
...
if (
  normalized === "せんとちひろのかみかくし" ||
  normalized === "セントチヒロノカミカクシ" ||
  normalized === "千と千尋の神隠し"
) {
...
function getTitleSearchScore(
  ...
  if (
    exactMatchedCandidates.size >= 2
  ) {

    score +=
      exactMatchedCandidates.size *
      20000;
...
async function getNetflixNewWorks(
  apiKey,
  country,
  genre,
  netflixType
